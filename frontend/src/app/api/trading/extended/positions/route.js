...
export async function GET(req) {
  ...
  const market = searchParams.getAll('market'); // Support multiple markets
  const side = searchParams.get('side'); // LONG or SHORT
  ...
  market.forEach(m => { if (m) params.append('market', m); });
  if (side) params.append('side', side);

  for (const base of bases()) {
    try {
      const r = await fetch(`${base}/extended/trading/positions?${params}`, {
        method: 'GET',
